...
def theoretical_error_parabolic(res,epsilon,t,y):
    ...
    # The theoretical error in the y-direction
    L2_error =  (t**2)*(exp(epsilon)-1)**2  + (res**2)*(factor**2)
    # Finally we can return the predicted theoretical error
    return L2_error#/(exp(2*epsilon)*(t**2 + y**2)) 
#=================================================================================
#=================================================================================
...
axs_1[0].legend(loc='best',prop={"size":10})
axs_1[0].set_ylabel('Explanatory variable, $y(t)$',fontsize=15)
axs_1[0].set_xlabel('Response variable, $t$',fontsize=15)
# Change the size of the ticks
axs_1[0].tick_params(axis='both', which='major', labelsize=10)
axs_1[0].tick_params(axis='both', which='minor', labelsize=10)
...
# Legends and axes labels
axs_1[1].legend(loc='best',prop={"size":10})
axs_1[1].set_ylabel('Explanatory variable, $y(t)$',fontsize=15)
axs_1[1].set_xlabel('Response variable, $t$',fontsize=15)
# Change the size of the ticks
axs_1[1].tick_params(axis='both', which='major', labelsize=10)
axs_1[1].tick_params(axis='both', which='minor', labelsize=10)
# Title and saving the figure
...
for epsilon in epsilon_vec_parabola:
    ...
    # Transform the data points with the given transformation parameter
    y_trans = array([(y-C_p2_opt_data_circle)*exp(epsilon)+C_p2_opt_data_circle for index,y in enumerate(data_circle)])
    # Extract the temporary parameter
    ...
    residuals = array([C_hat*(t_temp**2)+C_p2_opt_data_circle for t_temp in t_trans])-y_trans + (t_circle-t_trans)
...
for epsilon in epsilon_vec_parabola:
    ...
    # Transform the data points with the given transformation parameter
    y_trans = array([(y-C_p2_opt_data_parabola)*exp(epsilon)+C_p2_opt_data_parabola for index,y in enumerate(data_parabola)])
    # Extract the temporary parameter
    C_temp = popt_data_parabola_fit_parabola[0]
    # Calculate the transformed parameter C in y=Ct^2
    C_hat = C_temp*exp(-epsilon)
    # Next, we calculate the residuals
    residuals = array([C_hat*(t_temp**2)+C_p2_opt_data_parabola for t_temp in t_trans])-y_trans +  (t_parabola-t_trans)
    # Lastly, we append the sum of squares
...
#Legends and axes labels
axs_2[0].legend(loc='best',prop={"size":10})
axs_2[0].set_ylabel('Fit, $SS(\\epsilon)$',fontsize=15)
axs_2[0].set_xlabel('Transformation parameter, $\\epsilon$',fontsize=15)
#Change the size of the ticks
axs_2[0].tick_params(axis='both', which='major', labelsize=10)
axs_2[0].tick_params(axis='both', which='minor', labelsize=10)
#Title and saving the figure
...
axs_2[1].plot(epsilon_vec_parabola,SS_data_parabola_fit_parabola, '-', label="Fitted parabola" ,color=(103/256,0/256,31/256),linewidth=3.0)
# The theoretical prediction of the cubic model
axs_2[1].plot(epsilon_vec_circle,SS_theo_data_parabola_fit_parabola, 'D', label="Theoretical parabola" ,color=(152/256,0/256,67/256),linewidth=5.0)
# Add a lovely grid
axs_2[1].grid()
# Set the limits
...
# Legends and axes labels
axs_2[1].legend(loc='best',prop={"size":10})
axs_2[1].set_ylabel('Fit, $SS(\\epsilon)$',fontsize=15)
axs_2[1].set_xlabel('Transformation parameter, $\\epsilon$',fontsize=15)
# Change the size of the ticks
axs_2[1].tick_params(axis='both', which='major', labelsize=10)
axs_2[1].tick_params(axis='both', which='minor', labelsize=10)
# Title and saving the figure
